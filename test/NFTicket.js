...
// })
  ...
  it("Get Role",async function(){
    const {hardhatNFTicket,owner,admin,issuer,student,hardhatPortfolio}=await loadFixture(deployFixture);
    const reply=await hardhatNFTicket.getRole(owner.address);
    console.log(reply);

    await hardhatNFTicket.connect(issuer).registerIssuer("name","pfp","email");
    await hardhatPortfolio.connect(student).registerStudent("name","pfp","email");
    const reply2=await hardhatNFTicket.getRole(issuer.address)
    console.log(reply2)
